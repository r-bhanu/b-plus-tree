...
node *init_L(int d)
{
   ...
   for (int i = 0; i < 2; i++)
   {
      np->child_ptr[i] = nullptr;//child_ptr[0] = left sibling, child_ptr[1] = right sibling
   }
   np->parent = nullptr;
   leaf_nodes_num++;
   return np;
...
void InsertionInBTree(node *curr, node *newptr, int separtor, int t)
{
   ...
   else
   {
      ...
      if (curr->n < 2 * t + 1)
      {
         ...
         int count = (curr->n);
         while (curr->data[count - 1] > separtor and count > 0)
         {
            // data k liye
      ...
      else
      {
         ...
         if (curr->data[t - 1] < separtor and separtor < curr->data[t])//curr is the index node to be spiltted
         {
            for (int i = 0; i < t + 1; i++)
            {
   ...
   else
   {
      ...
      else // split leaf node
      {
         ...
         if (curr->data[d - 1] < value)
         {
            for (int i = 0; i < d; i++)
            {
                //moving data to new node(d values)
